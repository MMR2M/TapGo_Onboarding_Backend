...
MAX_FILES = 10
BLANK_SIGNATURE_DATA = "data:image/png;base64,iVBORw0KGgo="
...
def validate_payload(payload: dict[str, Any], require_contract_acceptance: bool = True) -> tuple[dict[str, Any], list[str]]:
    errors: list[str] = []
    ...
    cleaned = {
        ...
        "ethernet": ethernet,
        "sla": sla,
        "signatureData": signature_data or BLANK_SIGNATURE_DATA,
...
def format_contract_datetime(payload: dict[str, Any]) -> str:
    label = (payload.get("clientDateTime") or "").strip()
    timezone_name = (payload.get("clientTimeZone") or "").strip()
    if label and timezone_name:
...
def render_contract_html(reference: str, payload: dict[str, Any], submission_dir: Path) -> Path:
    ...
    context = {
        ...
        "generated_at": format_contract_datetime(payload),
        "signed_at": format_contract_datetime(payload),
    }
...
def preview_contract() -> Any:
    ...
    cleaned, errors = validate_payload(payload, require_contract_acceptance=False)
    if errors:
